...
__global__ void search_kernel(const int *arr,
    const int len, const int search, int *res, bool *flag)
{
    if(*flag == false) {
        int tid = threadIdx.x;
...
int binary_search(const int *arr, const int len, const int search)
{
    int *d_arr, *d_res;
    bool *d_flag;

    size_t arr_size = len * sizeof(int);
    size_t res_size = sizeof(int);
    ...
    search_kernel<<<gridSize,blockSize>>>(d_arr, len, search, d_res, d_flag);
